...
// Optional: GET single workshop by ID
export async function getWorkshopById(id) {
  try {
    ...
    if (workshop.program_type === 'pace') {
      const { data: paceData, error: paceError } = await supabaseAdmin
        .from('paceworkshops')
        .select('category')
        .eq('workshop_id', id)
        .single();

      if (!paceError && paceData) {
        workshop.category = paceData.category;
      }
    } else if (workshop.program_type === 'non_pace') {
      const { data: nonPaceData, error: nonPaceError } = await supabaseAdmin
        .from('nonpaceworkshops')
        .select('csc')
        .eq('workshop_id', id)
        .single();

      if (!nonPaceError && nonPaceData) {
        workshop.csc = nonPaceData.csc;
      }
    }
